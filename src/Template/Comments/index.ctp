...
<div class="comments index large-10 medium-9 columns">
    ...
    <?php foreach ($comments as $comment): ?>
        <tr>
            ...
            <td>
                <?= $comment->has('job') ? $this->Html->link($comment->job->id, ['controller' => 'Jobs', 'action' => 'view', $comment->job->id]) : '' ?>
            </td>
            <td>
                <?= $comment->has('user') ? $this->Html->link($comment->user->id, ['controller' => 'Users', 'action' => 'view', $comment->user->id]) : '' ?>
            </td>
            <td><?= $this->Number->format($comment->flagged) ?></td>
            <td><?= h($comment->date_created) ?></td>
            <td class="actions">
